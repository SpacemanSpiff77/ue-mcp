...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    read_state_ledger_fingerprint: bp("Read-only deterministic SHA-256 definition fingerprint for a Blueprint, UserDefinedStruct, or UserDefinedEnum. Blueprint hashing traverses exact live graph topology (including layout) plus structural metadata locally but returns only the compact hash. It never compiles, saves, or reconstructs. Params: assetPath, assetType", "read_state_ledger_fingerprint", (p) => ({ assetPath: p.assetPath ?? p.path, assetType: p.assetType })),
  ...
  undefined,
  {
    ...
    assetType: z.enum(["Blueprint", "UserDefinedStruct", "UserDefinedEnum"]).optional().describe("read_state_ledger_fingerprint: resolved State Ledger asset type"),
