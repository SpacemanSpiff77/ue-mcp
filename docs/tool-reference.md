...
| `read_function_topology` | Deterministic, read-only topology for one exact case-sensitive Blueprint function. Returns all nodes, pins, native GUIDs where Unreal supplies them, complete pin types/defaults, exact wire endpoints, references, unresolved endpoints, bounds, and package dirty-state evidence. Never compiles, saves, reconstructs, or modifies the Blueprint. Params: `assetPath, functionName, maxNodes?, maxPins?, maxConnections?` |
