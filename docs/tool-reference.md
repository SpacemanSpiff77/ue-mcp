...
| `read_function_topology` | Deterministic, read-only, exact case-sensitive function graph topology. Returns native graph/node/pin GUIDs where Unreal supplies them, complete pin types/defaults, exact wire endpoints, references, unresolved endpoints, bounds, and package dirty-state evidence. It never compiles, saves, reconstructs, or modifies the Blueprint. Params: `assetPath, functionName, maxNodes?, maxPins?, maxConnections?` |
| `read_blueprint_topology` | Deterministic, bounded, read-only complete Blueprint graph inventory and exact topology from one Unreal observation. Returns small results inline and freezes larger qualified results for immutable multipart delivery; non-byte bounds remain all-or-nothing. Params: `assetPath, maxAuthoredGraphs?, maxNodesPerGraph?, maxPinsPerGraph?, maxConnectionsPerGraph?, maxTotalNodes?, maxTotalPins?, maxTotalConnections?, maxSerializedBytes?` |
| `read_blueprint_topology_chunk` | Fetch one deterministic base64 chunk from frozen Full Blueprint capture memory without re-reading Unreal. Params: `captureHandle, chunkIndex` |
| `release_blueprint_topology_capture` | Release frozen Full Blueprint capture memory without reading or mutating Unreal. Params: `captureHandle` |
