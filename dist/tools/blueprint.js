...
export const blueprintTool = categoryTool("blueprint", "Blueprint reading, authoring, and compilation, including AnimGraph/EventGraph node scripting: add_node, connect_pins, search_node_types, plus variables, functions, graphs, components, interfaces, and event dispatchers.", {
    ...
    read_function_topology: {
        ...bp("Deterministic, read-only, exact case-sensitive function graph topology. Returns native graph/node/pin GUIDs where Unreal supplies them, complete pin types/defaults, exact wire endpoints, references, unresolved endpoints, bounds, and package dirty-state evidence. It never compiles, saves, reconstructs, or modifies the Blueprint. Params: assetPath, functionName, maxNodes?, maxPins?, maxConnections?", "read_blueprint_function_topology", (p) => ({ path: p.assetPath, functionName: p.functionName, maxNodes: p.maxNodes, maxPins: p.maxPins, maxConnections: p.maxConnections })),
        timeoutMs: 180_000,
    },
    read_blueprint_topology: {
        ...bp("Deterministic, bounded, read-only complete Blueprint graph inventory and exact topology. Pointer-deduplicates all UE 5.8 Blueprint graph collections, audits owned UEdGraph objects, classifies exclusions and unsupported graphs explicitly, and returns small topology inline or an immutable multipart capture manifest. Non-byte bounds remain all-or-nothing. Params: assetPath, maxAuthoredGraphs?, maxNodesPerGraph?, maxPinsPerGraph?, maxConnectionsPerGraph?, maxTotalNodes?, maxTotalPins?, maxTotalConnections?, maxSerializedBytes?", "read_blueprint_topology", (p) => ({
            ...
            maxPinsPerGraph: p.maxPinsPerGraph,
            maxConnectionsPerGraph: p.maxConnectionsPerGraph,
            maxTotalNodes: p.maxTotalNodes,
            maxTotalPins: p.maxTotalPins,
            maxTotalConnections: p.maxTotalConnections,
            maxSerializedBytes: p.maxSerializedBytes,
        })),
        timeoutMs: 180_000,
    },
    ...
    author: {
        ...
        handler: async (ctx, p) => {
            const assetPath = p.assetPath;
            if (!assetPath)
                throw new Error("Missing 'assetPath'");
            const steps = [];
            const run = async (step, target, method, params) => {
                try {
                    const result = await ctx.bridge.call(method, params);
                    steps.push({ step, target, ok: true, result });
                }
                catch (e) {
                    steps.push({ step, target, ok: false, error: e instanceof Error ? e.message : String(e) });
                }
            };
            let created = false;
            if (p.parentClass) {
                try {
                    const r = await ctx.bridge.call("create_blueprint", { path: assetPath, parentClass: p.parentClass });
                    created = r?.created !== false;
                    steps.push({ step: "create", target: assetPath, ok: true, result: r });
                }
                catch (e) {
                    steps.push({ step: "create", target: assetPath, ok: false, error: e instanceof Error ? e.message : String(e) });
                }
            }
    ...
    maxNodes: z.number().int().positive().optional().describe("read_function_topology: all-or-nothing node bound (default 512, hard max 4096)"),
    maxPins: z.number().int().positive().optional().describe("read_function_topology: all-or-nothing pin bound (default 4096, hard max 32768)"),
    maxConnections: z.number().int().positive().optional().describe("read_function_topology: all-or-nothing connection bound (default 8192, hard max 65536)"),
    maxAuthoredGraphs: z.number().int().positive().optional().describe("read_blueprint_topology: authored graph bound (default 64, hard max 256)"),
    maxNodesPerGraph: z.number().int().positive().optional().describe("read_blueprint_topology: per-graph node bound (default 512, hard max 4096)"),
    maxPinsPerGraph: z.number().int().positive().optional().describe("read_blueprint_topology: per-graph pin bound (default 4096, hard max 32768)"),
    maxConnectionsPerGraph: z.number().int().positive().optional().describe("read_blueprint_topology: per-graph connection bound (default 8192, hard max 65536)"),
